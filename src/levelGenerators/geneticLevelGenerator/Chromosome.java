...
public class Chromosome implements Comparable<Chromosome>{
		...
		for(int y = 0; y < height; y++){
			for(int x = 0; x < width; x++){
				this.level[y][x] = new ArrayList<String>();
				 //System.out.println(this.level[y][x]); //it prints empty
			}
		}
	...
	public void InitializeConstructive(){
		...
		HashMap<Character, ArrayList<String>> charMap = SharedData.constructiveGen.getLevelMapping();
 		//System.out.println (levelString.length);
		for(int y=0; y<levelString.length; y++){
		//	System.out.println (levelString[y]);
			for(int x=0; x<levelString[y].length(); x++){
				if(levelString[y].charAt(x) != ' ')
				{
					//System.out.println (charMap.get(levelString[y].charAt(x)));
					this.level[y][x].addAll(charMap.get(levelString[y].charAt(x)));
 					// System.out.println (this.level[y][x]);
				}
			}
		}
		
		FixLevel();  //Fix the level by fixing the player number--there need to be only one avatar
		constructAgent(); // initialize agent to be used for evaulation of chromosomes
		
		 
		ArrayList<Chromosome> level_const= new ArrayList<Chromosome>();
	 //	level_const.add(<Chromosome>level.);      
 		// SharedData.CHROMO=levelString;
	...
	private void FixeDoor()
	{
		//get the list of all the avatar names
		ArrayList<SpriteData> eDoor = SharedData.gameDescription.geteDoor();
		ArrayList<String> eDoorNames = new ArrayList<String>();
 		
		for(SpriteData e:eDoor){
			eDoorNames.add(e.name);
		}
		
		eDoorNames.add("eDoor");
		//get list of all the avatar positions in the level
		ArrayList<SpritePointData> eDoorPositions = getPositions(eDoorNames);
		
		// if not avatar insert a new one 
		if(eDoorPositions.size() == 0){
			ArrayList<SpritePointData> freePositions = getFreePositions(eDoorNames);
			
			int index = SharedData.random.nextInt(freePositions.size());
			System.out.println((eDoorNames.size()));
			level[freePositions.get(index).y][freePositions.get(index).x].add(eDoorNames.get(SharedData.random.nextInt(eDoorNames.size())));
		}

		//if there is more than one avatar remove all of them except one
		else if(eDoorPositions.size() > 1){
			int notDelete = SharedData.random.nextInt(eDoorPositions.size());
			int index = 0;
			for(SpritePointData point:eDoorPositions){
				if(index != notDelete){
					level[point.y][point.x].remove(point.name);
				}
				index += 1;
		...
		for(int y = 0; y < level.length; y++){
			for(int x = 0; x < level[y].length; x++){
				if(levelMapping.getCharacter(level[y][x]) == null)
				{
					...
					c= level[y][x].toString().charAt(1)  ;   //level[x][y].charAt(0)
	...
	public ArrayList<Double> calculateFitness(long time){
		if(!calculated){
			...
			constraint.addConstraints(new String[]{"SolutionLengthConstraint", "DeathConstraint", "CoverPercentageConstraint", "SpriteNumberConstraint", "GoalConstraint", "AvatarNumberConstraint", "WinConstraint"});
	...
	public void calculateNovelty()
	{
		...
		ArrayList[][]  st = new ArrayList[16][16];
		ArrayList[]  ConLevel = new ArrayList[16];
		 String conc="";
	 ...
	 if (chromo==null)
		 ...
		 for (int i=0; i<16 ; i++)
		 {
			 ...
			 ArrayList<String> stringList = new ArrayList<String>(Arrays.asList(conc)); //new ArrayList is only needed if you absolutely need an ArrayList
			 ...
			 conc="";
				 ...
				 for (int i = 0; i < 16; i++) 
				 {
					  cost=cost + editDistance (lines[i],ConLevel[i].toString());
	 ...
	 else if (chromo.get(0).level.length>0)
	 {
		 ...
		 { 
			 ...
			 levelM = chromo.get(i).level  ;
			 for (int j=0; j<levelM.length; j++)
			 {   
				 for (int k=0;k<levelM[0].length   ;k++)
				 {
				 
			               		conc=conc+ levelM[j][k];			               		
				 }
				 				ArrayList<String> stringList = new ArrayList<String>(Arrays.asList(conc)); //new ArrayList is only needed if you absolutely need an ArrayList
				 				ConLevel[j]=  stringList  ;
				 				conc="";
				 				System.out.println( ConLevel[j]);
				 				cost=cost + editDistance (lines[j],ConLevel[j].toString()); 	  
			 }			 
			 noveltyValue=(float)cost/(16*16);
			 PopNoveltyValue.add(i,noveltyValue);
			 System.out.println(PopNoveltyValue.get(i).toString());
			 //chromo.add(new Chromosome(PopNoveltyValue[i], noveltyValue));
			 
			SharedData.ITERATION_NOVELTY = noveltyValue;
			 System.out.println(noveltyValue);
	...
	public static int editDistance(String s1, String s2) {
	    ...
	    System.out.println(costs[16]); 
	    return costs[16]  ;// costs[s2.length()];
	  }

...
public class LevelGenerator extends AbstractLevelGenerator{
	...
	private ArrayList<Chromosome> getNextPopulation(ArrayList<Chromosome> fPopulation, ArrayList<Chromosome> iPopulation){
		ArrayList<Chromosome> newPopulation = new ArrayList<Chromosome>();
		 LevelMapping levelM = new LevelMapping(SharedData.gameDescription); // Getting some null values otherwise right file
		...
		while(newPopulation.size() < SharedData.POPULATION_SIZE){
			...
			Chromosome parent1 = rouletteWheelSelection(population); // correct sprite names
			Chromosome parent2 = rouletteWheelSelection(population); // correct sprite names
			...
			if(SharedData.random.nextDouble() < SharedData.CROSSOVER_PROB){
				...
				child1 = children.get(0); // correct sprite names
				child2 = children.get(1); // correct sprite names
			...
			newPopulation.add(child1);  // correct sprite names
			newPopulation.add(child2);  // correct sprite names
		}
		...
		if(fPopulation.isEmpty()){
 		...
 		SharedData.CHROMO=newPopulation;
	...
	public String generateLevel(GameDescription game, ElapsedCpuTimer elapsedTimer) {
		...
		numOfInFeasible = new ArrayList<Integer>();
          ...
          SharedData.gameDescription = game;
		 LevelMapping levelM = new LevelMapping(SharedData.gameDescription);
		
		int size = 0;
		if(SharedData.gameAnalyzer.getSolidSprites().size() > 0){ size = 2; // Solid Sprits = Avatar
		}
